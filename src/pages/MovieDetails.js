...
export function MovieDetails(props) {
    ...
    useEffect(() => {
        async function fetchDetails(params) {
            const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?language=hi-IN&region=IN&api_key=b80d59c33d6d57ed9c7e3713f91c188a`);
        ...
        <main>
            <section className='flex justify-around flex-wrap py-5'>
                <div className="max-w-sm">
                    <img className='rounded' src={imagePath} alt={data.original_title}></img>
                </div>
                <div className='max-w-2xl text-gray-700 text-lg dark:text-white'>
                    <h1 className='text-4xl font-bold my-3 text-centre lg:text-left'>{data.original_title}</h1>
